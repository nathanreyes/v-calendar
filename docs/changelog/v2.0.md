...
### Enhancements
* Add `left-side` and `right-side` scoped slots

The `left-side` and `right-side` slots may be used to place your own custom content at the left part of the calendar.

<guide-readme-dp-left-side />

<guide-readme-dp-right-side />
